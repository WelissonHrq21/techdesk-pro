...
describe("TechDesk setup bootstrapper", () => {
  ...
  it.runIf(hasShell())(
    "keeps env, logs and backups private while metadata remains readable",
    () => {
      const result = runShellCommand(
        [
          'runtime="$(mktemp -d)"',
          ...
          'chmod 666 "$runtime/.env" "$runtime/techdesk-installation.json"',
          'DEPLOY_ROOT="$runtime"',
          ...
          'setup_log_init',
          'printf "env=%s\\nmetadata=%s\\nlogs=%s\\nbackups=%s\\nlog=%s\\n" "$(stat -c %a "$runtime/.env")" "$(stat -c %a "$runtime/techdesk-installation.json")" "$(stat -c %a "$runtime/logs")" "$(stat -c %a "$runtime/backups")" "$(stat -c %a "$SETUP_LOG_FILE")"',
        ].join("\n"),
        [setupCoreScript]
      );
