...
docker compose --project-directory "$RUNTIME" -p "$PROJECT" --env-file "$RUNTIME/.env" -f "$RUNTIME/docker-compose.yml" up -d postgres >/dev/null
ready=0
for _attempt in $(seq 1 30); do
  if docker compose --project-directory "$RUNTIME" -p "$PROJECT" --env-file "$RUNTIME/.env" -f "$RUNTIME/docker-compose.yml" exec -T postgres pg_isready -U techdesk -d techdesk >/dev/null 2>&1; then ready=1; break; fi
  sleep 1
done
[ "$ready" -eq 1 ] || { echo "Isolated PostgreSQL did not become ready." >&2; exit 1; }
